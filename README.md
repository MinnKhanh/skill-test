...
- When adding a new `guard` to the system, you need to init and add `Factory` to `app/Factories`.
...
The project follows a structured documentation pattern located in the `docs/` directory. This ensures that both developers and AI agents can quickly understand the system's logic, architecture, and features.

- **`docs/requirements.md`**: Initial system requirements. Consult this only when specifically requested as it may become outdated compared to the structured documentation in `docs/system/`.
- **`docs/ai-context.md`**: A central index and "map" specifically designed for AI agents to navigate the documentation.

### 1. System Logic & Architecture (`docs/system/`)
Contains high-level technical documentation for the core of the project:
- **`overview.md`**: Brief introduction to the system, tech stack, and main features.
- **`technical-architecture.md`**: Details the software architecture, including service layers and data flow.
- **`domain-model.md`**: Conceptual entities and their relationships (Ubiquitous Language).
- **`database-design.md`**: Physical database schema, tables, and column details.
- **`business-rules.md`**: Core business constraints and global validation rules.
- **`interface-design.md`**: Specification for both UI and API endpoints.

### 2. Feature Implementations (`docs/features/`)
Detailed technical breakdowns for specific modules or features.

### 3. API Documentations (`docs/api/`)
Detailed API specifications and standards, supplementing the automated Scramble documentation.
...
- When adding a new guard, add its API prefix to `$apiBasePaths` variable in `app/Providers/AppServiceProvider` to include its routes in the API documentation.